...
<div class="card">
    <div class="card-body">
        ...
        <form action="{{route('Seccion.destroy',$seccion->id_seccion)}}" method="POST">
            <h5>{{$seccion->grado->nombre_grado}} "{{$seccion->nombre_seccion}}" DE {{$seccion->grado->nivel->nombre_nivel}} </h5>
            @method('delete')
            @csrf
            <p class="card-text">No podras recuperar la informacion una vez eliminada.</p>
            <button type="submit" class="btn btn-danger"><i class="fas fa-check-square"></i> SI </button>
            <a href="{{ route('CancelarSeccion')}}" class="btn btn-primary"><i class="fas fa-times-circle"></i> NO</button></a>
        </form>
    </div>
</div>
@endsection